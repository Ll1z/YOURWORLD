...
- 第 1 周：裸循环 + 最小空间工具，能回答最简单的「某点周围有什么」
- 第 2 周：CodeAct 沙箱 + traceback 自修复 + 空间自检器
- 第 3 周：多源数据接入（OSM + 本地 + 边界）+ 出图与报告
- 第 4 周：评测集（30 到 50 题 + 纯 SQL 生成的 ground truth）+ 看板 + MCP 封装 + 消融实验（无 RAG / 单向量库 / 混合 + 空间过滤）
