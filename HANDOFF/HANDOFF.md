...
- 复核者判「重做」的标准是封闭的四条：答非所问 / 结论没有工具支撑 / 该说明的没说明 / 有子问题没答。不封闭的话它会退化成随机的答案改写器，比没有还糟
- 重做最多一次；而且只有「复核问题更少」时才顶替原答案，否则保留上一版并写明原因——复核者的意见是线索，不是命令
- 入口：`scripts/ask_agent.py --team`、`scripts/run_eval.py --mode team`。单循环仍是默认路径

对照实验（同一批 10 条：nearby-001/006/009/019、agg-003/006/011、dist-002、places-001、refusal-001）：

| 模式 | 两次抽样通过数 | 10 题 token |
|---|---|---|
| 单循环 | 10/10（子集单跑）、8/10（全量那次里的同一批） | 335k |
| 多 Agent | 7/10、9/10 | 436k、370k |

- 结论：这批样本上多 Agent 没有可测量的增益，代价是约 1.3 倍 token 与更多步数（规划者要多搜一轮经验库）。所以它保持 opt-in
- 复核者第一次出手就抓到一个真问题：执行者步数触顶时交出空答案——工具结果都拿到了，却什么都没说。洞补在执行者侧：触顶后补一次不带工具的收口（`loop._finalize`），把已有工具结果讲清楚。补完之后这批用例里复核者没有再抓到新问题
- 三次失败样本里 rework=0、复核给的都是 pass，说明失败来自执行者自身（同名地名选点、模型自己拿距离做减法），不是协作机制引入的。复核者查的是语义错位，查不了「中心点选得和冻结期望值不一样」——那是评测集与复核者各自的口径差异，不是 bug
- 顺带修掉的：`loop.run` 触顶时空答案（见上）；`--mode team` 的汇总行不再自称「agent 层」
